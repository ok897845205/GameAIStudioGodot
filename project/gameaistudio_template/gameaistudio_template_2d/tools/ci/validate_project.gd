...
func _init() -> void:
	...
	if main_scene == null:
		...
		if not main is Node2D:
			errors.append("Main scene root must be Node2D.")
		for node_path in ["Player", "Player/CollisionShape2D", "Player/Sprite2D", "Ground", "Ground/CollisionShape2D", "CanvasLayer/HUD"]:
