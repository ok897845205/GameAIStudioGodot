// Minimal subset of tool types needed by the streaming engine.
 ...
 * type so call sites can share a compact, predictable contract.
 */
 ...
 *
 * GameAIStudio never authors or executes client-side tools (its agents run
 * local CLIs), so this keeps only the permissive shape the model-context
 * registry needs to merge/spread tool entries.
 */
export type Tool<
